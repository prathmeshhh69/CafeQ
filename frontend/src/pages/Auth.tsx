  ...
  return (
    <AuthFrame headline={"Good food.\nBetter moods."} mode="login">
      <motion.div variants={formItemVariants} className="mb-7">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange/20 bg-orange/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.13em] text-orange">
          <Sparkles className="h-3.5 w-3.5" /> Your CafeQ corner
        </div>
      ...
      <form onSubmit={submit} className="space-y-5">
        <motion.div variants={formItemVariants}>
          <Input name="identifier" label="Email or phone number" icon={<Mail className="h-4 w-4" />} placeholder="you@example.com" autoComplete="username" required className="auth-input !py-3" />
        </motion.div>
        <motion.div variants={formItemVariants}>
          <PasswordInput name="password" label="Password" placeholder="Enter your password" autoComplete="current-password" required />
        </motion.div>
        <motion.div variants={formItemVariants} whileHover={loading ? undefined : { y: -2 }} whileTap={loading ? undefined : { scale: 0.99 }} animate={{ opacity: loading ? 0.84 : 1 }} transition={{ duration: 0.16 }}>
          <Button type="submit" size="lg" block loading={loading} className="auth-primary !mt-7 !py-3.5">
            Log In <ArrowRight className="h-4 w-4" />
          </Button>
        </motion.div>
      </form>
      <motion.p variants={formItemVariants} className="mt-7 text-center text-sm text-muted">
        New here?{" "}
        <button type="button" onClick={() => go("register")} className="auth-link font-semibold text-ink underline decoration-orange/60 decoration-2 underline-offset-4 hover:text-orange">Create an account</button>
      </motion.p>
    </AuthFrame>
  );
}
  ...
  return (
    <AuthFrame headline={"Join the\nCafeQ family!"} mode="register">
      <motion.div variants={formItemVariants} className="mb-6">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange/20 bg-orange/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.13em] text-orange">
          <Sparkles className="h-3.5 w-3.5" /> A seat at our table
        </div>
      ...
      <form onSubmit={submit} className="space-y-4">
        <motion.div variants={formItemVariants}>
          <Input name="name" label="Full name" icon={<User className="h-4 w-4" />} placeholder="Ojas Rane" autoComplete="name" required className="auth-input !py-3" />
        </motion.div>
        <motion.div variants={formItemVariants}>
          <Input name="email" label="Email address" icon={<Mail className="h-4 w-4" />} type="email" placeholder="you@example.com" autoComplete="email" required className="auth-input !py-3" />
        </motion.div>
        <motion.div variants={formItemVariants}>
          <Input name="phone" label="Phone number" icon={<Phone className="h-4 w-4" />} type="tel" placeholder="+91 98765 43210" autoComplete="tel" required className="auth-input !py-3" />
        </motion.div>
        <motion.div variants={formItemVariants}>
          <PasswordInput name="password" label="Password" placeholder="Create a password" autoComplete="new-password" required />
        </motion.div>
        <motion.div variants={formItemVariants} whileHover={loading ? undefined : { y: -2 }} whileTap={loading ? undefined : { scale: 0.99 }} animate={{ opacity: loading ? 0.84 : 1 }} transition={{ duration: 0.16 }}>
          <Button type="submit" size="lg" block loading={loading} className="auth-primary !mt-6 !py-3.5">
            Create Account <ArrowRight className="h-4 w-4" />
          </Button>
        </motion.div>
      </form>
      <motion.p variants={formItemVariants} className="mt-6 text-center text-sm text-muted">
        Already have an account?{" "}
        <button type="button" onClick={() => go("login")} className="auth-link font-semibold text-ink underline decoration-orange/60 decoration-2 underline-offset-4 hover:text-orange">Log in</button>
      </motion.p>
    </AuthFrame>
  );
}
