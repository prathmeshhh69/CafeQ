...
export function Logo({ onClick, size = "md" }: { onClick?: () => void; size?: "sm" | "md" }) {
  return (
    <button onClick={onClick} className="group inline-flex items-center gap-2" aria-label="CafeQ home">
      <span className="grid h-9 w-9 place-items-center rounded-xl bg-ink text-lime transition-transform group-hover:-rotate-6">
...
export function Navbar({
  ...
  const link = (r: string, label: string) => {
    ...
    return (
      <button
        onClick={() => go(r)}
        className={`relative rounded-lg px-3 py-2 text-sm font-medium transition-colors ${active ? "text-ink" : "text-muted hover:text-ink"}`}
      >
        {label}
    ...
    <header className="sticky top-0 z-40 border-b border-line bg-cream/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-[1280px] items-center gap-4 px-4 sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <button onClick={onSearch} className="hidden h-10 w-10 place-items-center rounded-xl text-muted hover:bg-black/5 hover:text-ink sm:grid" aria-label="Search menu">
            <Search className="h-5 w-5" />
          </button>
          <button onClick={() => go("cart")} className="relative grid h-10 w-10 place-items-center rounded-xl text-ink hover:bg-black/5" aria-label={`Cart, ${cartCount} items`}>
            <ShoppingBag className="h-5 w-5" />
            {cartCount > 0 && (
              <span className="absolute -right-0.5 -top-0.5 grid h-5 min-w-5 place-items-center rounded-full bg-lime px-1 text-[11px] font-bold text-ink ring-2 ring-cream">
                {cartCount}
              </span>
            )}
          </button>
          {user ? (
            <div className="relative">
              <button onClick={() => setMenuOpen((o) => !o)} className="flex items-center gap-2 rounded-xl py-1.5 pl-1.5 pr-2 hover:bg-black/5">
            ...
            <button onClick={() => go("login")} className="rounded-xl bg-ink px-4 py-2 text-sm font-semibold text-cream hover:bg-black">Log In</button>
          )}
        </div>
      </div>
    </header>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-cream/95 backdrop-blur-md md:hidden">
      <div className="mx-auto flex max-w-md items-stretch justify-around px-2 pb-[env(safe-area-inset-bottom)]">
        {items.map(({ r, label, icon: Icon, badge }) => {
          const active = route === r || (r === "menu" && route === "home");
          return (
            <button key={r} onClick={() => go(r)} className={`relative flex flex-1 flex-col items-center gap-0.5 py-2.5 text-[11px] font-medium ${active ? "text-ink" : "text-muted"}`}>
